...
(async()=>{
 ...
 try {
  ...
  assert.equal(await page.locator('#metrics .metric').count(),8);assert.equal(await page.locator('#chart path').count(),4);assert.equal(await page.locator('#chart .chart-exposure-area').count(),1);assert.equal(await page.locator('#ranked tr').count(),1);assert.equal(await page.locator('#comparison-rows').count(),0);assert.equal(await page.locator('#exposure-rows').count(),0);assert.equal(await page.locator('#threshold-rows').count(),0);assert.equal(await page.locator('#scarcity-rows').count(),0);
