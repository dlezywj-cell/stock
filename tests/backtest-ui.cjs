...
(async()=>{
 ...
 try {
  ...
  assert.equal(await page.locator('#capital').inputValue(),'1000');assert.equal(await page.locator('#top-n').inputValue(),'10');assert.equal(await page.locator('#market').inputValue(),'CNY');assert.equal(await page.locator('#start').inputValue(),'2026-01-01');
  ...
  assert.equal(await page.locator('#metrics .metric').count(),8);assert.equal(await page.locator('#chart path').count(),3);assert.equal(await page.locator('#chart .chart-exposure-area').count(),1);assert.equal(await page.locator('.legend').textContent().then(t=>t.includes('原策略')),false);assert.equal(await page.locator('#ranked tr').count(),1);assert.equal(await page.locator('#comparison-rows').count(),0);assert.equal(await page.locator('#exposure-rows').count(),0);assert.equal(await page.locator('#threshold-rows').count(),0);assert.equal(await page.locator('#scarcity-rows').count(),0);
  for(const [selector,label] of [['#chart','策略净值'],['#drawdown','回撤']]) {await page.locator(selector).scrollIntoViewIfNeeded();const box=await page.locator(selector).boundingBox();await page.mouse.move(box.x+box.width/2,box.y+box.height/2);assert.equal(await page.locator(selector+' .chart-hover').getAttribute('visibility'),'visible');const tip=await page.locator(selector+' .chart-tooltip').textContent();assert.match(tip,/2026-/);assert.ok(tip.includes(label));assert.equal(await page.locator(selector+' .chart-crosshair').count(),2);await page.mouse.move(1,1);assert.equal(await page.locator(selector+' .chart-hover').getAttribute('visibility'),'hidden');}
