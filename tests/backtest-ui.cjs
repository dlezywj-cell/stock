...
(async()=>{
 ...
 try {
  ...
  const fields=['EntryScore','EntryRank','ExitScore','ExitRank','ExitReason','MAE','MFE'];assert.ok(savedResult.trades.length>0);for(const t of savedResult.trades)for(const key of fields)assert.ok(Object.hasOwn(t,key));assert.ok(!JSON.stringify(savedResult).includes('test-local-token'));
