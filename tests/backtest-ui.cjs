...
(async()=>{
 ...
 try {
  ...
  assert.equal(await page.locator('#metrics .metric').count(),8);assert.equal(await page.locator('#chart path').count(),3);assert.equal(await page.locator('#ranked tr').count(),2);assert.equal(await page.locator('#comparison-rows tr').count(),7);assert.equal(await page.locator('#exposure-rows tr').count(),28);assert.equal(await page.locator('#threshold-rows tr').count(),5);
  ...
  const fields=['EntryScore','EntryRank','ExitScore','ExitRank','ExitReason','MAE','MFE','DrawdownFromPeak'];assert.ok(savedResult.trades.length>0);assert.ok(savedResult.baseline.metrics);assert.equal(savedResult.exposure.length,28);assert.equal(savedResult.eligibleThresholdStats.length,5);for(const t of savedResult.trades)for(const key of fields)assert.ok(Object.hasOwn(t,key));assert.ok(!JSON.stringify(savedResult).includes('test-local-token'));
