...
(async()=>{
 ...
 try {
  ...
  const fields=['EntryScore','EntryRank','ExitScore','ExitRank','ExitReason','MAE','MFE','DrawdownFromPeak'];assert.ok(savedResult.trades.length>0);assert.ok(savedResult.baseline.metrics);assert.equal(savedResult.exposure.length,28);for(const t of savedResult.trades)for(const key of fields)assert.ok(Object.hasOwn(t,key));assert.ok(!JSON.stringify(savedResult).includes('test-local-token'));
  const exposureEvent=page.waitForEvent('download');await page.locator('#export-exposure').click();const exposureDownload=await exposureEvent;const exposureCSV=fs.readFileSync(await exposureDownload.path(),'utf8');assert.match(exposureCSV.split('\r\n')[0],/"TrendChange5"/);assert.equal(exposureCSV.split('\r\n').length,29);
  ...
  await page.locator('#rebalance-date').selectOption({index:1});assert.match(await page.locator('#rebalance-summary').textContent(),/续持 2 只/);assert.match(await page.locator('#ranked').textContent(),/续持/);assert.ok((await page.locator('#ranked').textContent()).includes('—'));
