...
(async () => {
    ...
    try {
        ...
        assert.equal((await state()).visible.length, 2);
        await page.screenshot({path:'/tmp/stock-watchlists-desktop.png'});
        await page.setViewportSize({width:390,height:844});
        await page.reload();
        await page.waitForFunction(() => stocks.length === 3);
        assert.equal(await page.locator('.watchlist-bar').isVisible(), true);
        assert.equal(await page.locator('#top-area').isVisible(), false);
        assert.equal(await page.evaluate(() => document.documentElement.scrollWidth <= innerWidth), true);
        await page.screenshot({path:'/tmp/stock-watchlists-mobile.png'});
